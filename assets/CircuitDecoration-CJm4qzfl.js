import{r as n,j as t}from"./vendor-three-OgilUm_S.js";import{d as c,e as u,h as x}from"./main-C208-Vam.js";const j=n.memo(({variant:l="sparse",opacity:p=.15,position:o="both"})=>{const[i,d]=n.useState(!1);n.useEffect(()=>{const r=window.matchMedia("(prefers-reduced-motion: reduce)");d(r.matches);const e=a=>d(a.matches);return r.addEventListener("change",e),()=>r.removeEventListener("change",e)},[]);const m=l==="dense"?12:6,f=l==="dense"?8:4,s=n.useMemo(()=>Array.from({length:m},(r,e)=>({id:`node-${e}`,x:10+Math.random()*80,y:10+Math.random()*80,size:3+Math.random()*4,color:[c,u,x][e%3],delay:e*.3,duration:3+Math.random()*2})),[m]),$=n.useMemo(()=>Array.from({length:f},(r,e)=>{const a=s[e%s.length],y=s[(e+2)%s.length];return{id:`conn-${e}`,x1:a.x,y1:a.y,x2:y.x,y2:y.y,color:a.color,delay:e*.5,duration:4+Math.random()*2}}),[s,f]),g=`

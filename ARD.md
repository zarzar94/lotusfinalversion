...
- **Backend (خدمة منفصلة)**: يتم استهلاك API خارجي عبر `VITE_API_URL` (مثلاً خدمة Express/Mongo منشورة على Render/Fly/AWS). توجد شيفرة مرجعية في مجلد `backend/` لكنها تُنشر كخدمة مستقلة وليست جزءًا من بناء الواجهة.
...
- **نماذج الخلفية (في الخدمة المنفصلة)**: Users/ClinicalProgress/Gamification/Settings/Session تُدار في الـ API الخارجي (انظر شيفرة مرجعية في `backend/` إذا لزم). الواجهة تتكامل فقط عبر REST.
...
- **Frontend (هذا المستودع)**: React/Vite ثابت، ينشر على GitHub Pages (أو أي استضافة static)، ويتواصل مع الـ API عبر `VITE_API_URL`.【F:src/services/api.ts†L36-L119】
- **Backend (خدمة منفصلة)**: يُنشر كخدمة مستقلة (Render/Fly/AWS) مع عقد Express/Mongo مماثلة للشيفرة المرجعية في `backend/`، لكنه ليس جزءًا من بناء الواجهة.
- **Database**: مزود مُدار (Mongo Atlas أو Postgres) مربوط بالخلفية.
- **التكامل**: الواجهة ترسل طلبات REST موقعة بـ JWT عبر `VITE_API_URL`، ويمكن استبدال مزود الخلفية طالما يحترم نفس العقد.
## 8) متطلبات تشغيل/نشر

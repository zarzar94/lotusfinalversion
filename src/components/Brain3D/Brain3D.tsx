  ...
  for (let y = 0; y <= heightSegments; y++) {
    ...
    for (let x = 0; x <= widthSegments; x++) {
      ...
      const centralFissure = Math.abs(px) < 0.15 && py > -0.3
        ? -0.15 * (1 - Math.abs(px) / 0.15) * Math.max(0, py + 0.3)
        : 0;
      py += centralFissure;
      ...
      const temporalBulge = py < 0 && Math.abs(px) > 0.4 ? 0.08 * Math.abs(px) * Math.abs(py) : 0;
      ...
      const cerebellumBulge = py < -0.4 && pz < -0.2
        ? 0.15 * Math.abs(py + 0.4) * Math.max(0, -pz - 0.2)
        : 0;

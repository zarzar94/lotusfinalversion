...
// Sound effect for accordion
const playToggleSound = (open: boolean) => {
  try {
    const ctx = new (window.AudioContext || (window as unknown as { webkitAudioContext: typeof AudioContext }).webkitAudioContext)();
    const osc = ctx.createOscillator();
    const gain = ctx.createGain();
    osc.type = 'sine';
    osc.frequency.setValueAtTime(open ? 600 : 400, ctx.currentTime);
    osc.frequency.exponentialRampToValueAtTime(open ? 800 : 300, ctx.currentTime + 0.08);
